...
void calculate_Stering() {
    ...
    bool angle1_aligned = AtTargetAngle(angle_front, target_angle_front, 5.5);
    bool angle2_aligned = AtTargetAngle(angle_rear_left, target_angle_rear_left, 5.5);
    bool angle3_aligned = AtTargetAngle(angle_rear_right, target_angle_rear_right, 5.5);
    ...
    bool Check_setzero() {
        ...
        if (abs(V_x) <= 0.0005 && abs(V_y) <= 0.0005 && abs(Omega_z) <= 0.0005) {
            setzero();
            return true;
        } else {
            return false;
